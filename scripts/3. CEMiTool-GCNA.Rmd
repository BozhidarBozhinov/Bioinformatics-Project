---
title: 'Adapted from CEMiTool: Co-expression Modules Identification Tool'
output:
  html_document:
    toc: yes
  pdf_document:
...
Preparing input from DESeq2
```{r}
dds$Treatment <- relevel(dds$agent, ref = "DMSO")
#Shrinkage to Log2 fold changes
#  First find what are the coef
resultsNames(dds)
sum(assay(dds)<10)
# Stabilizing count variance
vsd <- varianceStabilizingTransformation(dds) %>% assay() %>% as.data.frame()
class(vsd)
```

Converting gene annotation names in the required input format and annotating samples
```{r}
#Extracting sample annotation
sample_annot<-as.data.frame(colData(dds))
sample_annot<-cbind(rownames(sample_annot),as.character(sample_annot$agent))

colnames(sample_annot)<-c("SampleName","Class")
sample_annot<-as.data.frame(sample_annot)

# annotate/convert gene names differently
Genes2 <- AnnotationDbi::select(org.Hs.eg.db, keys = rownames(vsd),
  columns = c('SYMBOL'), keytype = 'ENSEMBL') %>% dplyr::distinct(ENSEMBL, .keep_all= TRUE) %>% dplyr::distinct(SYMBOL, .keep_all= TRUE) %>% na.omit()  #remove duplicates and N.A. values of ENSEMBL column
#reorder accordingly and remove features with duplicate meaning
vsd <- vsd[Genes2$ENSEMBL,]
#check overlap
all(rownames(vsd)==Genes2$ENSEMBL) #If true, go on
rownames(vsd)<- Genes2$SYMBOL

#renaming to match the vignette script;;;instead of --- >cemm<-data("expr0")
expr0<-vsd
head(expr0[,1:4])
...
```{r, results='hide'}
# If beta is not forced, data set GSE161860 is not amenable to automatic optimization
cem <- cemitool(expr0,sample_annot, force_beta=TRUE)
#automatic beta selection failed because CEMiTool was unable to find a range of beta values with R² values above 0.8. As I'm sure you're aware, higher R² values imply in higher adherence to the scale-free model, but higher beta values will result in lower network connectivity, so you have to decide how much of the trade-off you're willing to accept. 
```
...
cem <- cemitool(expr0, sample_annot, gmt_in, interactions=int_df, 
                filter=TRUE, plot=TRUE, verbose=TRUE, force_beta=TRUE) # Not sure if forcing beta is unappropriate
